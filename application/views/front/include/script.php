<script src="<?php echo base_url('assets/front/template/'); ?>plugins/bootstrap/js/bootstrap.bundle.min.js"></script>
<!-- AdminLTE App -->
<script src="<?php echo base_url('assets/front/template/'); ?>dist/js/adminlte.min.js"></script>
<script src="<?php echo base_url('assets/front/template/'); ?>plugins/ekko-lightbox/ekko-lightbox.min.js"></script>

<!-- AdminLTE for demo purposes -->
<script src="<?php echo base_url('assets/front/template/'); ?>dist/js/demo.js"></script>
<script src="<?php echo base_url('assets/front/template/'); ?>plugins/jquery-validation/jquery.validate.min.js"></script>
<script src="<?php echo base_url('assets/front/template/'); ?>plugins/jquery-validation/additional-methods.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/summernote@0.8.18/dist/summernote.min.js"></script><!-- jQuery -->
<script src="<?php echo base_url('assets/front/template/'); ?>plugins/dropzone/min/dropzone.min.js"></script>
...
FileInput.prototype = {
 ...
 _initTemplateDefaults: function () {
  ...
  tCaption = '<div tabindex="500" class=" {class}">\n' +
  ...
  '</div>\n';
            ...
            '</div>';
            //noinspection HtmlUnknownAttribute
            tActionDelete = '<button type="button" class="far fa-trash-alt kv-file-remove {removeClass}" ' +
            'title="{removeTitle}" {dataUrl}{dataKey}>{removeIcon}</button>\n';
            tActionUpload = '';
            tActionZoom = '<button type="button" class=" kv-file-zoom {zoomClass} fa fa-search-plus" ' +
...
_renderMain: function () {
 var self = this, dropCss = (self.isUploadable && self.dropZoneEnabled) ? 'p' : 'file-drop-disabled',
...
$.fn.fileinput.defaults = {
  ...
  browseClass: 'btn btn-outline-dark btn-circle',
...
<script>
  $(document).ready(function(){
     var sample_data = new Bloodhound({
        ...
        prefetch:'<?php echo base_url(); ?>Anasayfa/fetch',
        remote:{
           url:'<?php echo base_url(); ?>Anasayfa/fetch/%QUERY',
           wildcard:'%QUERY'
       }
   });
     ...
     $('#prefetch .typeahead').typeahead(null, {
        ...
        limit:20,
        templates:{
           suggestion:Handlebars.compile('<a href="<?php echo base_url('Anasayfa/profile/'); ?>{{id}}"><div  class="d-flex pb-1"><div class="col-md-12"><img src="<?php echo base_url(); ?>{{image}}" class="img-thumbnail" width="48"/>{{name}}</div></div><a/>')
       }
   });
 });
</script><!-- Bootstrap 4 -->
...
<script>
  $(document).ready(function(){
   ...
   {
    $.ajax({
       url:"<?php echo base_url(); ?>Anasayfa/fetchi",
       method:"POST",
...
<script type="text/javascript">
  function savelike(storyid)
  {
     $.ajax({
        type: "POST",
        url: "<?php echo site_url('Anasayfa/savelikes');?>",
        data: "Storyid="+storyid,
        success: function (response) {
           $("#like_"+storyid).html(response);
...
<script type="text/javascript">
  function saveCommetlike(storyid)
  {
     $.ajax({
        type: "POST",
        url: "<?php echo site_url('Anasayfa/saveCommetlike');?>",
        data: "Storyid="+storyid,
        success: function (response) {
           $("#like_"+storyid).html(response);
...
<script>
  $(document).ready(function(){
     ...
     {
        $.ajax({
           url:"<?php echo site_url('Anasayfa/ajaxi');?>",
           method:"POST",
           data:{view:view},
           dataType:"json",
           success:function(data)
           {
              $('.dropdown-menulike').html(data.notification);
    ...
    $(document).on('click', '.dropdown-togglelike', function(){
        $('.count').html('');
...
<script>
  $(document).ready(function(){
     ...
     {
        $.ajax({
           url:"<?php echo site_url('Anasayfa/follownoti');?>",
           method:"POST",
           data:{view:view},
           dataType:"json",
           success:function(data)
           {
              $('.dropdown-menuf').html(data.notificationf);
              if(data.unseen_notificationf > 0)
              {
                 $('.countf').html(data.unseen_notificationf);
             }
         }
     });
    }
    ...
    $(document).on('click', '.dropdown-togglef', function(){
        $('.countf').html('');
        load_unseen_notification('yes');
    });
...
<script>
  $(document).ready(function(){
     ...
     var limit = 7;
     var start = 0;
     var kosul = <?php echo  $this->uri->segment(3) ?>;
     var action = 'inactive';
     ...
     function lazzy_loader(limit)
     {
        ...
        for(var count=0; count<limit; count++)
        {
           outputp += '<div class="post_data">';
           outputp += '<p><span class="content-placeholder" style="width:100%; height: 30px;">&nbsp;</span></p>';
           outputp += '<p><span class="content-placeholder" style="width:100%; height: 100px;">&nbsp;</span></p>';
           outputp += '</div>';
       }
       $('#load_data_message').html(outputp);
   }

   lazzy_loader(limit);

   function load_datap(limit, start)
   {
    $.ajax({
       url:"<?php echo base_url(); ?>Anasayfa/fetchip",
       method:"POST",
       data:{limit:limit, start:start,kosul:kosul},
       cache: false,
       success:function(data)
       {
          if(data == '')
...
{
    action = 'active';
    load_datap(limit, start);
}

$(window).scroll(function(){
    if($(window).scrollTop() + $(window).height() > $("#load_datap").height() && action == 'inactive')
    {
       lazzy_loader(limit);
       action = 'active';
       start = start + limit;
...
<script>
  $(document).ready(function(){
     ...
     function lazzy_loader(limit)
     {
        ...
        for(var count=0; count<limit; count++)
        {
           outputfoll += '<div class="post_data">';
           outputfoll += '<p><span class="content-placeholder" style="width:100%; height: 30px;">&nbsp;</span></p>';
           outputfoll += '<p><span class="content-placeholder" style="width:100%; height: 100px;">&nbsp;</span></p>';
           outputfoll += '</div>';
       }
       $('#load_data_message').html(outputfoll);
   }

   lazzy_loader(limit);

   function load_dataf(limit, start)
   {
    $.ajax({
       url:"<?php echo base_url(); ?>Anasayfa/fetch_follow",
       method:"POST",
       data:{limit:limit, start:start,kosul:kosul},
       cache: false,
       success:function(data)
...
{
    action = 'active';
    load_dataf(limit, start);
}

$(window).scroll(function(){
    if($(window).scrollTop() + $(window).height() > $("#load_dataf").height() && action == 'inactive')
    {
       lazzy_loader(limit);
       action = 'active';
       start = start + limit;
...
<script>
  $(document).ready(function(){
     ...
     function lazzy_loader(limit)
     {
        ...
        for(var count=0; count<limit; count++)
        {
           outputfollowing += '<div class="post_data">';
           outputfollowing += '<p><span class="content-placeholder" style="width:100%; height: 30px;">&nbsp;</span></p>';
           outputfollowing += '<p><span class="content-placeholder" style="width:100%; height: 100px;">&nbsp;</span></p>';
           outputfollowing += '</div>';
       }
       $('#load_data_message').html(outputfollowing);
   }

   lazzy_loader(limit);

   function load_data_following(limit, start)
   {
    $.ajax({
       url:"<?php echo base_url(); ?>Anasayfa/fetch_following",
       method:"POST",
       data:{limit:limit, start:start,kosul:kosul},
       cache: false,
       success:function(data)
       {
         ...
         {
             $('#load_data_following').append(data);
             $('#load_data_message').html("");
             action = 'inactive';
         }
     }
...
{
    action = 'active';
    load_data_following(limit, start);
}

$(window).scroll(function(){
    if($(window).scrollTop() + $(window).height() > $("#load_data_following").height() && action == 'inactive')
    {
       lazzy_loader(limit);
       action = 'active';
       start = start + limit;
